...
//      System.Console.WriteLine(_index+":len:"+rhs_l);
  ...
  public production(
    ...
    string          action_str,
    int		    prec_num,
    int             prec_side): this(lhs_sym,rhs_parts,rhs_l,action_str)
    {
      /* Set the precedence */
  ...
  protected bool _nullable_known = false;

  /** Is the nullability of the production known or unknown? */
  public bool nullable_known() {return _nullable_known;}
   ...
   */
  protected static bool is_id_start(char c)
    {
   ...
   * @param c the character in question.
   */
  protected static bool is_id_char(char c)
    {
  ...
  protected string make_declaration(
				    ...
				    int     offset)
    {
      ...
      if (emit.lr_values())
        ret = "\t\tint " + labelname + "left = ((TUVienna.CS_CUP.Runtime.Symbol)" + 
	  emit.pre("stack") + ".elementAt(" + emit.pre("top") + 
	  "-" + offset + ")).left;\n" +
	  "\t\tint " + labelname + "right = ((TUVienna.CS_CUP.Runtime.Symbol)" + 
	  emit.pre("stack") + ".elementAt(" + emit.pre("top") +
	  "-" + offset + ")).right;\n";
      else ret = "";
  ...
  public bool set_nullable(bool v)
    {
  ...
  public override bool Equals(object other)
    {
      if (other.GetType()!=typeof(production))
	return false;
      else
