    ...
    public class SymbolTable
    {
        ...
        public IEnumerable<DataDefinition> GetAllEnclosingTypeReferences(TypeDefinition currentTypeDef)
        {
            ...
            while (symbolTable.CurrentScope >= Scope.Namespace) //Loop on enclosing scope until null scope. 
            {
                ...
                if (symbolTable.CurrentScope == Scope.Namespace && symbolTable.Programs.Any())
        ...
        /// <returns></returns>
        [CanBeNull]
        private List<T> GetFromTableAndEnclosing1<T>(string head,
            ...
            Func<SymbolTable, IDictionary<string, List<T>>> getTableFunction, Scope maxScope = Scope.Intrinsic) where T : Node
        {
            ...
            this.GetFromTableAndEnclosing2_(head, getTableFunction, result, maxScope);
            
            return result;
        }

        private void GetFromTableAndEnclosing2_<T>(string head,
           Func<SymbolTable, IDictionary<string, List<T>>> getTableFunction, List<T> result, Scope maxScope = Scope.Intrinsic) where T : Node
        {
            var table = getTableFunction.Invoke(this);
            GetFromTable2(head, table, result);
            if (EnclosingScope != null && EnclosingScope.CurrentScope >= maxScope)
            {
                EnclosingScope.GetFromTableAndEnclosing2_(head, getTableFunction, result, maxScope);
            }
        }
        ...
        [CanBeNull]
        private void GetFromTable2<T>(string head, IDictionary<string, List<T>> table, List<T> result) where T : Node
        {
            table.TryGetValue(head, out List<T> values);
        ...
        public IEnumerable<DataDefinition> GetVariables(SymbolReference symbolReference)
        {
            ...
            {
                return GetVariables(symbolReference.Name);
            }
        }
        ...
        private static IList<DataDefinition> EmptyDataDefinitionList = new ImmutableList<DataDefinition>();
        private static IList<TypeDefinition> EmptyList = new ImmutableList<TypeDefinition>();
